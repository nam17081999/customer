...
const SELECT_FIELDS = 'id,name,store_type,image_url,latitude,longitude,address_detail,ward,district,phone,phone_secondary,note,active,created_at,updated_at,is_potential,last_called_at,last_call_result,last_call_result_at,last_order_reported_at,sales_note,route_id'
...
let memCache = null        // { data: [...], count: number, cacheSavedAt: number, cacheVersion: number|null } | null
...
export async function setCachedStores(data, count, options = {}) {
  ...
  memCache = { data, count: normalizedCount, cacheSavedAt, cacheVersion }
  try {
    const db = await openDB()
    return new Promise((resolve) => {
      const tx = db.transaction(STORE_NAME, 'readwrite')
      const store = tx.objectStore(STORE_NAME)
      store.put({ data, count: normalizedCount, cacheSavedAt, cacheVersion }, CACHE_KEY)
...
async function _fetchAllNonDeleted(cached, options = {}) {
  try {
    ...
    await setCachedStores(all, all.length, { cacheVersion: serverVersion })
...
export async function appendStoreToCache(newStore) {
  ...
  try {
    ...
    await setCachedStores(updatedData, cached.count + 1, { cacheVersion: nextVersion })
    lastCountCheckAt = Date.now()
  } catch {
...
export async function appendStoresToCache(newStores) {
  ...
  try {
    ...
    const existingIds = new Set(cached.data.map((store) => store?.id).filter(Boolean))
    const storesToAppend = newStores.filter((store) => store?.id != null && !existingIds.has(store.id))
    if (storesToAppend.length === 0) return

    const nextVersion = cached.cacheVersion != null ? cached.cacheVersion + 1 : null
    await setCachedStores([...cached.data, ...storesToAppend], cached.count + storesToAppend.length, { cacheVersion: nextVersion })
    lastCountCheckAt = Date.now()
  } catch {
    // silently ignore — next load will refetch
  }
...
export async function updateStoresInCache(updates) {
  ...
  try {
    ...
    updates.forEach((item) => {
      if (item?.id != null) {
        const prev = updatesById.get(item.id) || {}
        updatesById.set(item.id, { ...prev, ...item })
      }
    })
    ...
    let updatedCount = 0
    const nextData = cached.data.map((store) => {
      const patch = updatesById.get(store?.id)
