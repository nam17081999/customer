...
async function _fetchOrValidate() {
  ...
  const cached = await getCachedStores()
  const cachedLength = Array.isArray(cached?.data) ? cached.data.length : -1
  if (cached && cachedLength === dbCount) {
    // Keep cache metadata aligned to actual data length.
    if (cached.count !== cachedLength) {
      await setCachedStores(cached.data, cachedLength)
    }
    return cached.data
  }
