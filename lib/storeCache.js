...
import { appendE2EStoreOverride, getE2EStoresOverride } from '@/lib/e2e-test-mode'
...
const SELECT_FIELDS = 'id,name,store_type,latitude,longitude,address_detail,ward,district,phone,phone_secondary,note,active,created_at,updated_at,is_potential,last_called_at,last_call_result,last_call_result_at,last_order_reported_at,sales_note'
...
let lastFreshValidationAt = 0 // timestamp when cache was last confirmed fresh
let inflightPromise = null // dedup: shared promise for concurrent callers
let mutationSequence = 0   // incremented on every local mutation; used to detect concurrent mutations during reconciliation


function markCacheFresh(timestamp = Date.now()) {
  lastCountCheckAt = timestamp
  lastFreshValidationAt = timestamp
}

function markVersionChecked(timestamp = Date.now()) {
  lastVersionCheckAt = timestamp
}

function resetFreshValidation() {
  lastCountCheckAt = 0
  lastVersionCheckAt = 0
  lastFreshValidationAt = 0
}
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = () => {
        ...
        memCache = {
          ...
          needsServerReconcile: Boolean(result.needsServerReconcile),
        }
          resolve(memCache)
        } else {
          resolve(null)
...
export async function setCachedStores(data, count, options = {}) {
  ...
  const needsServerReconcile = Boolean(options.needsServerReconcile)
  memCache = { data, count: normalizedCount, cacheSavedAt, cacheVersion, lastSyncedAt, needsServerReconcile }
  try {
    const db = await openDB()
    return new Promise((resolve) => {
      const tx = db.transaction(STORE_NAME, 'readwrite')
      const store = tx.objectStore(STORE_NAME)
      store.put({ data, count: normalizedCount, cacheSavedAt, cacheVersion, lastSyncedAt, needsServerReconcile }, CACHE_KEY)
...
async function _fetchOrValidate() {
  const now = Date.now()
  const skipCheck = memCache && !memCache.needsServerReconcile && (now - lastFreshValidationAt < COUNT_CHECK_COOLDOWN_MS)
  ...
  if (cached && !cached.needsServerReconcile && (now - (cached.cacheSavedAt || 0) < COUNT_CHECK_COOLDOWN_MS)) {
    markCacheFresh(now)
    markVersionChecked(now)
    return cached.data
  }

  if (hasCache && cached.needsServerReconcile) {
    const serverVersion = await getServerStoreVersion()
    const seqBefore = mutationSequence
    const result = await _fetchChangedNonDeletedSince(cached, { serverVersion })
    if (seqBefore !== mutationSequence && memCache) {
      memCache.needsServerReconcile = true
    }
    return result
  }
  ...
  if (!hasCache) {
    const serverVersion = await getServerStoreVersion()
    const seqBefore = mutationSequence
    const result = await _fetchAllNonDeleted(cached, { serverVersion })
    if (seqBefore !== mutationSequence && memCache) {
      memCache.needsServerReconcile = true
    }
    return result
  ...
  const seqBefore = mutationSequence
  const result = await _fetchAllNonDeleted(cached)
  if (seqBefore !== mutationSequence && memCache) {
    memCache.needsServerReconcile = true
  }
  return result
...
export async function getOrRefreshStores() {
  const e2eStores = getE2EStoresOverride()
  if (e2eStores.hasOverride) return e2eStores.stores

  // Dedup: if a request is already in-flight, piggyback on it
  if (inflightPromise) return inflightPromise

  const promise = _fetchOrValidate().finally(() => {
...
export async function appendStoreToCache(newStore) {
  if (!newStore) return
  if (appendE2EStoreOverride(newStore)) return
  mutationSequence++
  try {
    const cached = await getCachedStores()
    ...
    await setCachedStores(updatedData, updatedData.length, {
...
export async function updateStoreInCache(storeId, patch) {
  ...
  try {
    ...
    const nextVersion = cached.cacheVersion != null ? cached.cacheVersion + 1 : null
    await setCachedStores(nextData, cached.count, {
      cacheVersion: nextVersion,
      lastSyncedAt: cached.lastSyncedAt || null,
      needsServerReconcile: true,
    })
