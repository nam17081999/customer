...
export function getE2EGeolocationOverride() {
  ...
  if (!state || !Object.prototype.hasOwnProperty.call(state, 'geolocation')) {
    return { hasOverride: false, coords: null, error: null, heading: null, delayMs: 0 }
  }
  ...
  return {
    ...
    delayMs: Number(geolocation.delayMs || 0),
  }
}
...
export function appendE2EMapCameraEvent(event) {
  const state = getE2ETestState()
  if (!state || typeof event !== 'object' || event == null) return 0

  if (!state.map || typeof state.map !== 'object') {
    state.map = {}
  }
  if (!Array.isArray(state.map.cameraEvents)) {
    state.map.cameraEvents = []
  }

  state.map.cameraEvents.push(cloneValue(event))
  return state.map.cameraEvents.length
}
