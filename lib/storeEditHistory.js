...
export async function logStoreEditHistoryBatch(rows) {
  ...
  const { error } = await supabase.from('store_edit_history').insert(payloads)
  ...
  for (let i = 0; i < payloads.length; i += CHUNK_SIZE) {
    ...
    const { error: chunkErr } = await supabase.from('store_edit_history').insert(chunk)
    if (chunkErr) {
      ...
      for (const insertedIdx of insertedChunks) {
        ...
        await supabase.from('store_edit_history').delete().in('store_id', chunkIds).catch(() => {})
      }
