...
export async function logStoreEditHistory({ storeId, actionType, actorUserId, actorRole, changes }) {
  ...
  const { error } = await db.from('store_edit_history').insert([payload])
...
export async function logStoreEditHistoryBatch(rows) {
  ...
  // Try single insert first (atomic)
  const { error } = await db.from('store_edit_history').insert(payloads)
  if (!error) return { ok: true }

  // Single insert failed — likely row limit exceeded. Try chunked.
  const CHUNK_SIZE = 100
  const insertedChunks = []
  for (let i = 0; i < payloads.length; i += CHUNK_SIZE) {
    const chunk = payloads.slice(i, i + CHUNK_SIZE)
    const { error: chunkErr } = await db.from('store_edit_history').insert(chunk)
    if (chunkErr) {
      // Chunk failed — best-effort clean up previously inserted chunks
      for (const insertedIdx of insertedChunks) {
        const chunkIds = payloads.slice(insertedIdx, insertedIdx + CHUNK_SIZE).map((r) => r.store_id)
        await db.from('store_edit_history').delete().in('store_id', chunkIds).catch(() => {})
      }
      return { ok: false, error: chunkErr, partialError: `chunk ${i / CHUNK_SIZE} failed` }
    }
