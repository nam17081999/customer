...
import { IMAGEKIT_URL_ENDPOINT, IMAGEKIT_PUBLIC_KEY } from './constants';

// Server-side ImageKit instance (for uploads and management)
// Private key is read from process.env directly — never exposed to client
let imagekit = null;
...
export function getImageKitInstance() {
  if (!imagekit && typeof window === 'undefined') {
    ...
    if (!privateKey) {
      console.error('IMAGEKIT_PRIVATE_KEY env var is missing');
    }
