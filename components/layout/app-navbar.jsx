          ...
          <div className="ml-auto hidden items-center gap-1 sm:flex">
            {navLinks.map(({ href, active, label, Icon, badge }) => (
                ...
                >
                  ...
                  {active && <span className="absolute inset-x-2.5 -bottom-0.5 h-0.5 rounded-full bg-[color:var(--primary)]" />}
        ...
        <div className="mx-auto flex h-14 w-full max-w-screen-md">
          {navLinks.map(({ href, active, label, mobileLabel, Icon, badge }) => (
            <Link
              ...
              href={href}
              aria-current={active ? 'page' : undefined}
              className={`relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 px-0.5 transition-colors ${active ? 'text-[color:var(--primary)]' : 'text-gray-500 active:text-gray-200'}`}
              onFocus={(e) => e.currentTarget.classList.add('focus-visible-ring')}
              onBlur={(e) => e.currentTarget.classList.remove('focus-visible-ring')}
            >
              <Icon className="size-5 shrink-0" />
              <span className={`w-full truncate text-center text-[9px] font-medium leading-none whitespace-nowrap ${active ? 'text-[color:var(--primary)]' : 'text-gray-500'}`}>
                {mobileLabel || label}
              </span>
