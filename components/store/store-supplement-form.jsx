...
function SupplementMobileActionBar({ currentStep, geoBlocked, goToStep, handlePrimaryAction, primaryLabel, resolvingAddr, saving, stepCount }) {
  if (currentStep === 1) {
    return null
  }

  const previousStep = currentStep === 3 ? 2 : 1
  const disabled = currentStep === 3 ? saving || resolvingAddr || geoBlocked : saving
  const label = currentStep === 2 && stepCount !== 2 ? 'Tiếp theo →' : primaryLabel

  return (
    <div className="flex gap-2">
      <Button type="button" variant="outline" size="icon" icon={<span>←</span>} onClick={() => goToStep(previousStep)} />
      <Button type="button" className="flex-1" onClick={handlePrimaryAction} disabled={disabled}>
        {label}
      </Button>
    </div>
  )
...
function SupplementStepOne({ data }) {
  const {
    handlePrimaryAction,
    isSupplementMode,
    lockedChipClass,
...
function SupplementStepTwo({ data }) {
  const {
    active,
    addressDetail,
    district,
  ...
  return (
    <>
      ...
      <div className="hidden gap-2 pt-2 sm:flex">
        ...
        <Button type="button" className="flex-1" onClick={handlePrimaryAction} disabled={saving}>
          {stepCount === 2 ? primaryLabel : 'Tiếp theo →'}
        </Button>
      </div>
    </>
  )
...
function SupplementStepThree({ data }) {
  const {
    compassError,
    geoBlocked,
    goToStep,
...
export default function StoreSupplementForm({
  ...
  const primaryLabel = saving
    ...
    : currentStep < stepCount
      ? currentStep === 1 ? 'Tiếp theo' : 'Tiếp theo →'
      : finalSubmitLabel
  ...
  const mobileActionBar = currentStep === 1 ? (
    <div className="flex gap-2">
      {step1SecondaryLabel && onStep1SecondaryAction ? (
        <Button type="button" variant="outline" className="flex-1" onClick={onStep1SecondaryAction}>
          {step1SecondaryLabel}
  ...
  return (
    ...
    >
      {currentStep === 1 ? <SupplementStepOne data={viewData} /> : null}
      {currentStep === 2 ? <SupplementStepTwo data={viewData} /> : null}
      {currentStep === 3 ? <SupplementStepThree data={viewData} /> : null}
    </StoreStepFormLayout>
  )
}
