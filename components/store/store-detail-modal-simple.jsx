...
export default function StoreDetailModalSimple({ store, open, onOpenChange }) {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 z-100 flex items-center justify-center opacity-100 visible pointer-events-auto transition-[opacity,visibility] duration-250"
      onClick={(e) => { if (e.target === e.currentTarget) onOpenChange(false) }}
    >
      <div className="bg-[color:var(--surface)] rounded-lg w-full max-w-[480px] max-h-[80vh] overflow-y-auto border border-[color:var(--border)] scale-100 opacity-100 transition-[transform,opacity] duration-250 ease">
        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-4 pb-0">
          <h3 className="flex items-center gap-3 min-w-0 flex-1 text-[18px] font-semibold">
            <span className="shrink-0">{typeMeta.icon}</span>
            <span className="truncate">{store.name}</span>
          </h3>
          <button className="size-7 rounded-full bg-[color:var(--surface2)] border-none text-[color:var(--muted)] cursor-pointer grid place-items-center text-[16px] hover:text-[color:var(--fg)] shrink-0" onClick={() => onOpenChange(false)} aria-label="Đóng">✕</button>
        </div>
        ...
        {/* Actions */}
        <div className="flex gap-1.5 px-5 pb-5 flex-wrap">
