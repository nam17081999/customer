...
export default function SearchStoreCard({ store, distance, searchTerm, compact }) {
  ...
  // ── Compact horizontal layout - MAU SAC NOI BAT ──
  if (compact) {
    const compactCard = (
        <Card className="overflow-hidden rounded-2xl bg-gray-700 hover:bg-gray-600 transition duration-200 border-3 border-gray-500 cursor-pointer shadow-lg">
          <CardContent className="p-0">
            <div className="flex gap-4 p-4">
              {/* Thumbnail - LON HON */}
              <div className="relative w-24 h-24 rounded-xl overflow-hidden bg-gray-600 flex-shrink-0 border-2 border-gray-400">
                <Image
                  src={imageSrc}
                ...
                />
                {/* Badge da xac thuc - MAU XANH LA NOI BAT */}
                {store.active && (
                  <div className="absolute top-1 left-1">
                    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-green-500 text-white shadow-lg border-2 border-white">
              ...
              {/* Info - FONT LON, CONTRAST CAO */}
              <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
                <h3 className="font-bold text-white text-xl leading-snug line-clamp-1">
                  {renderHighlightedName(store.name, searchTerm)}
                </h3>
                <p className="text-base text-gray-200 line-clamp-2 leading-snug">{addressText}</p>
                
                {/* Khoang cach - NEN XANH DUONG */}
                {distance !== null && distance !== undefined && (
                  <span className="inline-flex items-center gap-1.5 text-base font-bold text-white bg-blue-600 rounded-lg px-2 py-1 w-fit">
                ...
                {/* Dien thoai - NEN XANH LA NOI BAT */}
                {store.phone && (
                  <a
                    href={`tel:${store.phone.replace(/\s+/g,'')}`}
                    className="inline-flex items-center gap-2 text-lg font-bold text-white bg-green-600 hover:bg-green-500 rounded-lg px-3 py-1.5 w-fit transition-colors"
              ...
              {/* Nut ban do - MAU CAM NOI BAT */}
              {store.latitude && store.longitude && (
                <div className="flex-shrink-0 flex items-center">
                  <button
                    type="button"
                    className="w-16 h-16 rounded-xl bg-orange-500 hover:bg-orange-400 flex items-center justify-center text-white shadow-lg transition-all border-2 border-orange-300"
