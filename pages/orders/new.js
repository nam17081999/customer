...
function readSavedSalesOrderDrafts() {
  ...
  try {
    return parseSalesOrderDraftStoragePayload(
      window.localStorage.getItem(SALES_ORDER_DRAFTS_STORAGE_KEY),
    );
  } catch {
    return null;
  }
...
export default function NewSalesOrderPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError("");
    try {
      const { products: productRows, stores: storeRows } =
        await loadSalesOrderEntryData();
      ...
      setDrafts((prev) => {
        ...
        const rawStoreId = Array.isArray(router.query.storeId)
          ? router.query.storeId[0]
          : router.query.storeId;
        const hasQuickOrderStore =
          rawStoreId &&
          (storeRows || []).some(
            (store) => String(store?.id) === String(rawStoreId),
          );
      ...
      setError(
        err?.operatorMessage ||
          err?.message ||
          "Không tải được dữ liệu lên đơn.",
      );
    } finally {
  ...
  const selectedCustomer = useMemo(
    () =>
      stores.find(
        (store) => String(store.id) === String(activeDraft?.customerStoreId),
      ),
    [stores, activeDraft?.customerStoreId],
  );
  const recentProducts = useMemo(
    () => getRecentProductsFromOrderDrafts(drafts, productsById, 6),
    [drafts, productsById],
  );
  ...
  useEffect(() => {
    if (
      ...
      return;
    const payload = buildSalesOrderDraftStoragePayload({
      drafts,
      activeDraftId,
    });
    try {
      if (payload) {
        window.localStorage.setItem(
          SALES_ORDER_DRAFTS_STORAGE_KEY,
  ...
  const consumeQuickOrderQuery = useCallback(() => {
    ...
    router.replace({ pathname: "/orders/new", query: nextQuery }, undefined, {
      shallow: true,
    });
  }, [router]);
  ...
  useEffect(() => {
    ...
    if (
      !draftStorageReady ||
      stores.length === 0 ||
      appliedQuickOrderStoreId === String(rawStoreId)
    )
      return;
  ...
  const filteredStores = useMemo(() => {
    const query = String(activeDraft?.customerQuery || "")
      .trim()
      .toLowerCase();
    ...
    return stores
      .filter(
        (store) =>
          storeLabel(store).toLowerCase().includes(query) ||
          String(store.phone || "").includes(query),
      )
      .slice(0, 8);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filterInventoryProducts(products, {
      query,
      excludeProductIds: (activeDraft?.items || []).map(
        (item) => item.productId,
      ),
    }).slice(0, 12);
  ...
  const setItem = (index, patch) => {
    ...
    const nextItems = activeDraft.items.map((item, itemIndex) =>
  ...
  const selectUnit = (index, unitId) => {
    ...
    if (priceType === "retail") {
      unitPrice =
        unit?.unit_retail_price ??
        product?.retail_price ??
        unit?.default_sale_price ??
        line?.unitPrice;
    } else if (priceType === "wholesale") {
      unitPrice =
        unit?.unit_wholesale_price ??
  ...
  const addProductLine = (productId = "") => {
    ...
    const nextLine = product
      ? createSalesOrderLine([product])
      : createSalesOrderLine(products);
  ...
  const removeLine = (index) => {
    if (!activeDraft) return;
    patchActiveDraft({
      items: activeDraft.items.filter((_, itemIndex) => itemIndex !== index),
    });
  };
  ...
  const handleCloseDraft = (draftId) => {
    const result = closeSalesOrderDraft({
      drafts,
  ...
  const showSuccessMessage = useCallback((text) => {
    ...
    window.setTimeout(() => {
      setMsgState((prev) =>
        prev?.text === text ? { ...prev, show: false } : prev,
      );
    }, 2500);
  }, []);
  ...
  const totals = useMemo(() => {
    ...
    const subtotal = items.reduce(
      (sum, item) =>
        sum + toNumber(item.quantity, 0) * toNumber(item.unitPrice, 0),
      0,
    );
  ...
  const stockIssues = useMemo(
    () => getSalesOrderStockIssues(activeDraft?.items || [], productsById),
    [activeDraft?.items, productsById],
  );

  const handleSubmit = async (event) => {
    event.preventDefault();
    if (submittingRef.current || submitting || !activeDraft) return;
    if (!activeDraft.customerStoreId || (activeDraft.items || []).length === 0)
      return;
    setError("");
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không tạo được đơn hàng.",
      );
      return;
    }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const requestId =
        activeDraft.requestId || createMutationRequestId("sales");
      ...
      const successText = `Đã lên đơn ${activeDraft.code || ""} thành công.`
        .replace(/\s+/g, " ")
        .trim();
    ...
    <>
      ...
      <main className="min-h-full  text-gray-100">
        ...
        >
          <div className="sticky top-0 z-40 rounded-md border border-gray-800 bg-gray-950 shadow-lg shadow-black/20">
                    ...
                    {filteredProducts.map((product) => (
                      ...
                      >
                        ...
                        <span className="block text-sm text-gray-400">
                          {product.sku || "Chưa có mã"} · Lẻ:{" "}
                          {formatMoney(product.retail_price || 0)} · Xỉ:{" "}
                          {formatMoney(product.wholesale_price || 0)}
                        </span>
                      </button>
                    ))}
                  </div>
            ...
            {recentProducts.length > 0 && !productQuery && (
              <div className="flex gap-2 overflow-x-auto border-t border-gray-900 px-3 py-2">
                <span className="shrink-0 py-2 text-sm text-gray-500">
                  Gần đây
                </span>
                {recentProducts.map((product) => (
                  <button
                    key={product.id}
                    type="button"
                    className="shrink-0 rounded-full border border-gray-800 bg-gray-900 px-3 py-2 text-sm text-gray-200 hover:border-gray-600"
                    onClick={() => addProductLine(product.id)}
                  >
                    {product.name}
                  </button>
          ...
          {error && (
            <div className="border-b border-red-900 bg-red-950 px-4 py-3 text-red-100">
              {error}
            </div>
          )}
          {stockIssues.length > 0 && !error && (
            <div className="border-b border-amber-900 bg-amber-950 px-4 py-3 text-amber-100">
              {stockIssues.map((issue) => issue.message).join("; ")}. Vui lòng
              giảm số lượng hoặc nhập hàng trước.
            </div>
          )}
          ...
          <div className="mt-3 grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_390px]">
            <section className="flex min-h-[520px] flex-col rounded-md border border-gray-800 bg-gray-950 lg:min-h-0">
              ...
              <div className="min-h-0 flex-1 overflow-auto p-2">
                {loading ? (
                  <div className="rounded-md border border-gray-800 bg-gray-900 p-4 text-gray-400">
                    Đang tải...
                  </div>
                ) : products.length === 0 ? (
                  <div className="rounded-md border border-gray-800 bg-gray-900 p-4 text-gray-400">
                  ...
                  <div className="rounded-md border border-gray-800 bg-gray-900 p-5 text-base text-gray-400">
                    Tìm hàng hóa ở ô phía trên rồi bấm vào kết quả để thêm vào
                    đơn.
                  </div>
                ) : (
                  (activeDraft?.items || []).map((item, index) => {
                    ...
                    return (
                      <div
                        key={`${activeDraft.id}-${item.productId}`}
                        className="mb-2 grid grid-cols-1 gap-2 rounded-md border border-gray-800 bg-gray-900 p-3 xl:grid-cols-[52px_44px_minmax(220px,1.5fr)_160px_120px_120px_150px_150px] xl:items-center"
                      >
                        ...
                        <div className="min-w-0 rounded-md border border-gray-800 bg-gray-950 px-3 py-2">
                          <span className="mb-1 block text-xs font-semibold uppercase text-gray-500 xl:hidden">
                            Tên hàng
                          </span>
                          <p className="truncate text-base font-semibold text-gray-100">
                            {product?.name || "Hàng hóa"}
                          </p>
                        ...
                        <label className="block">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              selectUnit(index, event.target.value)
                            }
                          >
                        ...
                        <div className="h-11 rounded-md border border-gray-800 bg-gray-950 px-3 py-2 text-right text-base font-bold text-gray-100">
                          <span className="float-left text-xs font-semibold uppercase text-gray-500 xl:hidden">
                            Thành tiền
                          </span>
              ...
              <div className="border-t border-gray-800 bg-gray-950 p-2">
                <div className="relative">
                  <Input
                    ...
                    onChange={(event) =>
                      patchActiveDraft({ note: event.target.value })
                    }
            ...
            <aside className="flex flex-col rounded-md border border-gray-800 bg-gray-950 p-4 lg:min-h-0">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="rounded-md bg-gray-800 px-3 py-2 text-base font-semibold">
                  NPP
                </div>
                    ...
                    <div className="absolute left-0 right-0 top-12 z-40 max-h-72 overflow-y-auto rounded-md border border-gray-700 bg-gray-950 shadow-xl">
                      {filteredStores.map((store) => (
                        ...
                        >
                          ...
                          <span className="block text-sm text-gray-400">
                            {[store.phone, store.ward, store.district]
                              .filter(Boolean)
              ...
              <div className="mt-6 space-y-4 text-base">
                ...
                <label className="flex items-center justify-between gap-3">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      patchActiveDraft({ discountAmount: event.target.value })
                    }
                  />
                </label>
