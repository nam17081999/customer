...
export default function SalesOrderDetailPage() {
  ...
  const customer = order
    ? storesById.get(String(order.customer_store_id))
    : null;
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      ...
      } = await loadSalesOrderDetailData(id);
  ...
  const handleCancel = async () => {
    if (
      ...
      return;
    if (!window.confirm(`Hủy đơn ${order.code}? Tồn kho sẽ được cộng lại.`))
      return;
    ...
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không hủy được đơn hàng.",
      );
    } finally {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    pageStyle:
      "@page { size: A4 landscape; margin: 0; } body { margin: 0; padding: 0; }",
  });
            ...
            {order && (
              <div>
                <h1 className="text-2xl font-bold">
                  Khách hàng: {customer?.name || "Không tìm thấy"}
                </h1>
                ...
                {customer && (
                  <p className="text-sm text-gray-500">
                    {[
                      ...
                      [customer.ward, customer.district].filter(Boolean)
                        .length > 0
                        ? `Địa chỉ: ${[customer.ward, customer.district].filter(Boolean).join(", ")}`
                        : "",
                    ]
                      .filter(Boolean)
                      .join(" · ")}
            ...
            <div className="space-y-4">
              <Card>
                <CardContent className="p-0">
                  ...
                  {(invoice?.lines?.length > 0 ? invoice?.lines : items)?.map(
                    (item) => {
                      if (item.productName) {
                        return (
                          <div
                            key={item.id}
                            className="grid grid-cols-1 gap-2 border-b border-gray-900 px-4 py-4 last:border-b-0 md:grid-cols-[1.5fr_0.55fr_0.75fr_0.9fr]"
                          >
                            <div>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 border-t border-gray-800 px-4 py-3">
                    ...
                    <div className="shrink-0 rounded-md border border-gray-800 bg-gray-900 px-5 py-3">
                      ...
                      <div
                        className={`flex items-baseline justify-between gap-8 ${Number(order.discount_amount) > 0 ? "mt-2" : ""}`}
                      >
