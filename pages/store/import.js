...
import { bulkInsertStores, updateStoreAndResolveDuplicate } from "@/api/stores/store-client";

const loadStoreImportFlow = () => import("@/helper/storeImportFlow");

function buildRowAddress(values) {
  return [values.addressDetail, values.ward, values.district]
    .filter(Boolean)
    .join(", ");
}
...
function getRowStatusVariant(status) {
  if (status === "ready")
    return "border-green-900/70 bg-green-950/20 text-green-200";
  if (status === "duplicate")
...
export default function StoreImportPage() {
  ...
  const loadExistingStores = useCallback(async () => {
    ...
    try {
      ...
      setExistingStores([]);
      setParseError(
        "Không tải được dữ liệu cửa hàng hiện có để kiểm tra trùng.",
      );
    } finally {
  ...
  const summary = useMemo(() => {
    ...
    const duplicate = previewRows.filter(
  ...
  const parseFileToPreview = useCallback(
    async (file) => {
      setParsing(true);
      setParseError("");
      setImportResult("");
      ...
      try {
        ...
        const { previewRows: nextPreviewRows, error } =
        ...
        setPreviewRows([]);
        setParseError(
          "Không đọc được file CSV. Vui lòng kiểm tra lại file mẫu.",
        );
      } finally {
        setParsing(false);
      }
  ...
  const handleChooseDuplicateTarget = useCallback(
    async (rowNumber, selectedDuplicateId) => {
      const { finalizePreviewRow } = await loadStoreImportFlow();
      setPreviewRows((prev) =>
        prev.map((row) => {
          if (row.rowNumber !== rowNumber) return row;
          const nextSelectedDuplicateId =
            row.selectedDuplicateId === selectedDuplicateId
              ? null
              : selectedDuplicateId;
          const nextResolutionMode =
            nextSelectedDuplicateId == null ? "" : row.resolutionMode;
  ...
  const handleSetDuplicateResolution = useCallback(
    async (rowNumber, resolutionMode) => {
      const { finalizePreviewRow } = await loadStoreImportFlow();
      setPreviewRows((prev) =>
        prev.map((row) => {
          if (row.rowNumber !== rowNumber) return row;
          const nextResolutionMode =
            row.resolutionMode === resolutionMode ? "" : resolutionMode;
          return finalizePreviewRow(
            row,
            row.duplicateInFileRows,
            row.phoneDuplicateInFileRows || [],
  ...
  const handleImport = useCallback(async () => {
    ...
    const createRows = readyRows.filter(
      (row) =>
        row.resolutionMode === "create" || row.duplicateMatches.length === 0,
    );
    const resolvedDuplicateRows = readyRows.filter(
      (row) =>
    ...
    try {
      const { buildResolutionPatch, chunkArray, prepareExistingStores } =
        await loadStoreImportFlow();
      ...
      if (createPayloads.length > 0) {
        for (const chunk of chunkArray(createPayloads, 100)) {
          const data = await bulkInsertStores(chunk);
      ...
      for (const row of resolvedDuplicateRows) {
        const existingStore = existingStores.find(
          (store) => store.id === row.selectedDuplicateId,
        );
        ...
        const data = await updateStoreAndResolveDuplicate(row.selectedDuplicateId, patch);
      ...
      setImportResult(
        `Đã xử lý thành công ${readyRows.length} dòng: ${nextInsertedStores.length} dòng tạo mới, ${resolvedDuplicateRows.length} dòng cập nhật từ cửa hàng nghi trùng. Bỏ qua ${skippedRows} dòng lỗi hoặc chưa chọn hướng xử lý.`,
      );
      setPreviewRows([]);
      setSelectedFileName("");
      ...
      setImportResult("");
      setParseError(
        error?.message || "Nhập dữ liệu thất bại. Vui lòng thử lại.",
      );
    } finally {
      setImporting(false);
    }
  ...
  if (loadingStores) {
    return (
      <FullPageLoading
        visible
        message="Đang tải dữ liệu để kiểm tra trùng..."
      />
    );
  }
            ...
            <CardContent className="space-y-4 p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <h1 className="text-xl font-bold text-gray-100">
                    Nhập nhiều cửa hàng từ file CSV
                  </h1>
                  <p className="mt-1 text-sm text-gray-400">
                    Tải file mẫu, điền dữ liệu theo đúng cột rồi tải lên để kiểm
                    tra lỗi và nghi trùng trước khi nhập.
                  </p>
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-gray-800 bg-black/40 p-3">
                  ...
                  <p className="mt-1 text-sm text-gray-400">
                    `Tên cửa hàng`, `Xã / Phường`, `Quận / Huyện`
                  </p>
                </div>
                <div className="rounded-xl border border-gray-800 bg-black/40 p-3">
                  <h2 className="text-base font-semibold text-gray-100">
                  ...
                  <p className="mt-1 text-sm text-gray-400">
                    Loại:{" "}
                    {STORE_TYPE_OPTIONS.map((option) => option.label).join(
                      ", ",
                    )}
                  </p>
                </div>
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                  ...
                  <Card
                    key={item.label}
                    className="rounded-2xl border border-gray-800 bg-gray-950"
                  >
                ...
                <CardContent className="space-y-4 p-4 sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        Mỗi dòng được kiểm tra lỗi dữ liệu, trùng y hệt trong
                        file và cửa hàng có thể trùng trong hệ thống cùng quận /
                        huyện.
                      </p>
                      <p className="mt-1 text-sm text-gray-500">
                        Đang hiển thị {visiblePreviewRows.length} /{" "}
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        {importing
                          ? "Đang nhập dữ liệu..."
                          : `Nhập ${summary.ready} dòng hợp lệ`}
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {visiblePreviewRows.map((row) => {
                      const selectedDuplicate = row.duplicateMatches.find(
                        (match) => match.id === row.selectedDuplicateId,
                      );
                          ...
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                  className={`inline-flex rounded-full px-2.5 py-1 text-sm font-medium ${getRowStatusVariant(row.status)}`}
                                >
                              ...
                              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                <div className="min-w-0">
                                  <p className="text-sm font-medium text-gray-100">
                                    Xử lý cửa hàng nghi trùng
                                  </p>
                                  <p className="mt-1 text-sm text-gray-400">
                                    Bước 1: chọn đúng cửa hàng trong hệ thống.
                                    Bước 2: chọn ưu tiên dữ liệu cũ hay dữ liệu
                                    mới.
                                  </p>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleToggleDuplicateList(row.rowNumber)
                                    }
                                  >
                                    {row.duplicateListExpanded
                                      ? "Ẩn danh sách trùng"
                                      : `Chọn trong ${row.duplicateMatches.length} cửa hàng`}
                                  </Button>
                                  {canResolveSystemDuplicate(row) && (
                                    <Button
                                      type="button"
                                      variant={
                                        row.resolutionMode === "create"
                                          ? "primary"
                                          : "outline"
                                      }
                                      className="h-9 px-3 text-sm"
                                      onClick={() =>
                                        handleSetDuplicateResolution(
                                          row.rowNumber,
                                          "create",
                                        )
                                      }
                              ...
                              {row.resolutionMode === "create" &&
                                canResolveSystemDuplicate(row) && (
                                  <div className="mt-3 rounded-lg border border-green-900/70 bg-green-950/20 px-3 py-2 text-sm text-green-200">
                                    Dòng này sẽ được tạo mới dù đang có cửa hàng
                                    nghi trùng trong hệ thống.
                                  </div>
                                )}
                              ...
                              {row.duplicateListExpanded && (
                                <div className="mt-3 space-y-2">
                                  {row.duplicateMatches.map((match) => (
                                    <div
                                      key={`${row.rowNumber}-match-${match.id}`}
                                ...
                                row.resolutionMode !== "create" && (
                                  <div className="mt-3 rounded-lg border border-sky-900/70 bg-sky-950/20 px-3 py-2">
                                    <p className="text-sm font-medium text-sky-100">
                                      Đã chọn đối chiếu với:{" "}
                                      {selectedDuplicate.name}
                                    </p>
                                    <p className="mt-1 text-sm text-sky-200/90">
                                      Field chỉ có ở một bên sẽ luôn được giữ
                                      lại. Với field có ở cả hai bên, chọn cách
                                      ưu tiên bên dưới.
                                    </p>
                                  </div>
                                )}
                                  ...
                                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                                    ...
                                    >
                                      ...
                                      <div className="mt-1 text-sm text-gray-400">
                                        Chỉ lấy thêm các field đang thiếu trong
                                        hệ thống.
                                      </div>
                                    </button>
                                    <button
                                    ...
                                    >
                                      ...
                                      <div className="mt-1 text-sm text-gray-400">
                                        Giữ lại field thiếu và ưu tiên dữ liệu
                                        từ file khi có xung đột.
                                      </div>
                                    </button>
                                  </div>
                                )}
