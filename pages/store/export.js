...
import { fetchPageForExport } from "@/api/stores/store-client";
...
function buildContactsVcf(storesWithPhone) {
  const cards = storesWithPhone.flatMap((store) => {
    ...
    return phones.map((phone, phoneIndex) => {
      const contactName =
        phones.length > 1 ? `${name} (${phoneIndex + 1})` : name;
      const lines = [
        "BEGIN:VCARD",
        "VERSION:2.1",
...
export function StoreExportScreen({ mode = "all" }) {
  ...
  const loadStores = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        ...
        const data = await fetchPageForExport({
          fields: EXPORT_STORE_SELECT_FIELDS,
          from,
          to,
        });
  ...
  const contactsCount = useMemo(() => {
    return storesWithPhone.reduce(
      (sum, store) => sum + extractPhones(store.phone).length,
      0,
    );
  }, [storesWithPhone]);
  ...
  const exportContacts = async () => {
    ...
    const stamp = new Date().toISOString().slice(0, 10);
    await exportTextFile(
      `npp-ha-cong-contacts-${stamp}.vcf`,
      vcf,
                ...
                <div>
                  ...
                  {!isDataOnly && !isContactsOnly && (
                    <p className="text-base text-gray-400">
                      Xuất danh sách cửa hàng ra file Excel và file danh bạ điện
                      thoại.
                    </p>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="rounded-xl border border-gray-800 p-3 bg-gray-950">
                      <p className="text-sm text-gray-400">
                        Cửa hàng có số điện thoại
                      </p>
                      <p className="text-2xl font-semibold text-gray-100">
                        {storesWithPhone.length}
                      </p>
                    </div>
                  </div>
              ...
              {!isContactsOnly && (
                <div className="rounded-xl border border-gray-800 p-3 bg-gray-950 space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Bao gồm tất cả cửa hàng: tên, loại, địa chỉ, số điện
                      thoại, trạng thái, tọa độ, ngày tạo và ngày cập nhật.
                    </p>
                  </div>
                  <Button
                    type="button"
              ...
              {!isDataOnly && (
                <div className="rounded-xl border border-gray-800 p-3 bg-gray-950 space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Chỉ xuất cửa hàng có số điện thoại. Để tương thích iPhone,
                      mỗi số điện thoại sẽ được tạo thành 1 contact riêng.
                    </p>
                  </div>
                  <Button
                    type="button"
                    ...
                    disabled={loading || contactsCount === 0}
                  >
                    Xuất file danh bạ
                  </Button>
                </div>
