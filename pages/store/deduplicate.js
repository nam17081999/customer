...
import { mergePrimaryStore, softDeleteSecondaryStores } from "@/api/stores/store-client";
        ...
        for (let j = i + 1; j < activeStores.length; j++) {
          ...
          if (Number.isFinite(latA) && Number.isFinite(lngA)) {
            ...
            if (Number.isFinite(latB) && Number.isFinite(lngB)) {
              ...
              if (
                distance <= 0.1 &&
                isSimilarNameByWords(nameAWords, storeB.name)
              ) {
                isMatch = true;
              }
            }
          ...
          if (!isMatch) {
            if (
              containsAllInputWords(nameAWords, storeB.name) &&
              containsAllInputWords(
                extractWords(storeB.name || ""),
                storeA.name,
              )
            ) {
              isMatch = true;
  ...
  const handleMerge = async (clusterIndex, primaryStoreId) => {
    ...
    try {
      ...
      const allPhones = new Set(
        [primaryStore.phone, primaryStore.phone_secondary].filter(Boolean),
      );
      ...
      if (!primaryStore.phone && phonesArr.length > 0)
        updatesTemp.phone = phonesArr[0];
      if (!primaryStore.phone_secondary && phonesArr.length > 1) {
        if (updatesTemp.phone !== phonesArr[1])
          updatesTemp.phone_secondary = phonesArr[1];
      }

      // location merging - treat lat/lng as a pair; validate both before using
      if (
        !Number.isFinite(parseCoordinate(primaryStore.latitude)) ||
        !Number.isFinite(parseCoordinate(primaryStore.longitude))
      ) {
        const secWithLoc = secondaryStores.find(
          (s) =>
            Number.isFinite(parseCoordinate(s.latitude)) &&
            Number.isFinite(parseCoordinate(s.longitude)),
        );
        if (secWithLoc) {
      ...
      if (Object.keys(updatesTemp).length > 0) {
        const { error: pErr } = await mergePrimaryStore(primaryStore.id, { ...updatesTemp, updated_at: timestamp });
        if (pErr) throw pErr;
        await updateStoreInCache(primaryStore.id, updatesTemp);
      }

      const secondaryIds = secondaryStores.map((s) => s.id);
      const { error: xErr } = await softDeleteSecondaryStores(secondaryIds);
      if (xErr) throw xErr;
              ...
              <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Tìm và hợp nhất các cửa hàng trùng tên/vị trí
                  </p>
                </div>
                <Button
                  type="button"
          ...
          {clusters.length > 0 && (
            <div className="rounded-xl bg-amber-950/30 border border-amber-900 p-3 mb-4">
              <p className="text-sm text-amber-200">
                Phát hiện {clusters.length} nhóm cơ sở nghi trùng nhau.
              </p>
            </div>
          )}
          ...
          <div className="space-y-6">
            {clusters.map((cluster, cIndex) => {
              ...
              return (
                <div
                  key={cIndex}
                  className="p-3 bg-gray-900/40 rounded-xl border border-gray-800 space-y-3"
                >
                      ...
                      return (
                        ...
                        >
                          <CardContent className="p-3 space-y-2 relative">
                            <h3 className="font-semibold text-gray-100 text-base break-words pr-20">
                              {store.name}
                            </h3>
