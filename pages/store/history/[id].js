...
export default function StoreEditHistoryPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      setPageReady(false);
      void router.replace(
        `/login?from=${encodeURIComponent(router.asPath || `/store/history/${storeId}`)}`,
      );
      return;
    }
    if (!isAdmin) {
  ...
  const resolveStoreName = useCallback(async () => {
    ...
    try {
      ...
      const cachedMatch = cachedStores.find(
  ...
  const loadPage = useCallback(
    async (offset, { append = false } = {}) => {
      ...
      setError("");
      try {
        const { data, error: fetchError } = await fetchEditHistory(storeId, nextOffset, PAGE_SIZE);
          ...
          <Card className="rounded-2xl border border-gray-800">
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-between gap-3 text-sm text-gray-400">
                <span>
                  Đang hiển thị: {filteredItems.length} / {items.length}
                </span>
                {actionFilter !== "all" ||
                fieldFilter !== "all" ||
                        ...
                        {keys.map((key) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-sm text-gray-500 line-through break-words [overflow-wrap:anywhere]">
                                {fromValue}
                              </p>
                              <p className="text-sm text-green-300 break-words [overflow-wrap:anywhere]">
                                {toValue}
                              </p>
                            </div>
                          );
