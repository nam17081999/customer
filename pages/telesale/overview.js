...
import {
  formatLastCalledText,
  getTelesaleResultLabel,
  hasReportedOrder,
} from "@/helper/telesale";
...
function isResultStale(store) {
  ...
  if (gapMinutes == null)
    return Boolean(store?.last_called_at && !store?.last_call_result_at);
...
function getPriorityTime(store) {
  ...
  if (group === 1) {
    return getTimeValue(
      store?.last_called_at || store?.updated_at || store?.created_at,
    );
  }
  if (group === 5) {
    return getTimeValue(
      store?.last_order_reported_at ||
        store?.last_call_result_at ||
        store?.last_called_at,
    );
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            ...
            {store.last_call_result_at && (
              <span className="rounded-full bg-gray-800 px-2 py-1 text-gray-200">
                Cập nhật kết quả:{" "}
                {formatLastCalledText(store.last_call_result_at)}
              </span>
            )}
...
export default function TelesaleOverviewPage() {
  ...
  const summary = useMemo(() => {
    ...
    const neverCalled = potentialStores.filter(
      (store) => !store.last_called_at,
    );
    const needFollowUp = potentialStores.filter((store) =>
      ["goi_lai_sau", "khong_nghe", "khong_nghe_may"].includes(
        store.last_call_result,
      ),
    );
    const interestedStores = potentialStores.filter((store) =>
      ["con_hang", "quan_tam"].includes(store.last_call_result),
    );
      ...
      .filter((store) => {
        ...
        if (result === "da_len_don" || result === "da_bao_don") {
          return isOlderThanDays(
            store.last_order_reported_at || store.last_called_at,
            3,
          );
        }
        if (result === "con_hang" || result === "quan_tam") {
          return isOlderThanDays(
            store.last_call_result_at || store.last_called_at,
            2,
          );
        }
        return true;
  ...
  const handleOpenUpdate = useCallback(
    (storeId) => {
      if (!storeId) return;
      router.push(
        `/telesale/call/${storeId}?from=${encodeURIComponent("/telesale/overview")}`,
      );
    },
    [router],
  );
            ...
            <CardContent className="space-y-4 p-4 sm:p-5">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-xl border border-sky-900 bg-sky-950/25 p-3">
                  ...
                  <p className="mt-1 text-3xl font-bold text-sky-200">
                ...
                <div className="rounded-xl border border-green-900 bg-green-950/25 p-3">
                  ...
                  <p className="mt-1 text-3xl font-bold text-green-200">
              ...
              <CardContent className="space-y-3 p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-lg font-semibold text-gray-100">
                    Danh sách ưu tiên gọi
                  </h2>
                  <span className="text-sm text-gray-400">
                    {summary.queueStores.length} cửa hàng
                  </span>
                </div>
                {summary.queueStores.length === 0 ? (
                  <p className="text-sm text-gray-400">
                    Chưa có cửa hàng cần ưu tiên gọi.
                  </p>
                ) : (
            ...
            <Card className="rounded-2xl border border-gray-800">
              <CardContent className="space-y-3 p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-sm text-gray-400">
                    {summary.recentCalls.length} cửa hàng
                  </span>
                </div>
                {summary.recentCalls.length === 0 ? (
                  <p className="text-sm text-gray-400">
                    Chưa có cuộc gọi nào được cập nhật.
                  </p>
                ) : (
                  <div className="space-y-3">
