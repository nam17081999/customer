...
import {
  TELESALE_CALL_RESULT_OPTIONS,
  formatLastCalledText,
  getTelesaleResultLabel,
} from "@/helper/telesale";
import { saveCallResult } from "@/api/stores/store-client";
...
export default function TelesaleCallResultPage() {
  ...
  const saveTelesaleUpdate = async () => {
    ...
    const { error: updateError } = await saveCallResult(store.id, updates);
  ...
  if (authLoading || !pageReady || loadingStore) {
    return (
      <FullPageLoading visible message="Đang chuẩn bị màn kết quả gọi..." />
    );
  }
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {store.phone && (
              <Button
                type="button"
                variant="outline"
                onClick={() => startPhoneCall(store.phone)}
              >
                Gọi lại
              </Button>
            )}
          ...
          <Card className="rounded-2xl border border-gray-800 bg-gray-950">
            <CardContent className="space-y-4 p-4 sm:p-5">
              <div className="space-y-1">
                ...
                {addressText && (
                  <p className="text-sm text-gray-400">{addressText}</p>
                )}
              </div>

              <div className="flex flex-wrap gap-2 text-xs text-gray-300">
                <span className="rounded-full bg-gray-800 px-2.5 py-1">
                  Lần gọi gần nhất: {formatLastCalledText(store.last_called_at)}
                </span>
                <span className="rounded-full bg-gray-800 px-2.5 py-1">
                  Kết quả cũ: {getTelesaleResultLabel(store.last_call_result)}
                </span>
              ...
              <div className="flex items-center justify-between gap-3 rounded-xl border border-gray-800 bg-gray-950 px-3 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Ưu tiên cửa hàng này cho telesale
                  </p>
                </div>
                <button
                  type="button"
