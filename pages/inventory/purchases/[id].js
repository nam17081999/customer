...
export default function PurchaseOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace(`/login?from=/inventory/purchases/${id || ""}`);
      return;
    }
    if (!isAdmin) {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setError("");
    try {
      const { detail, products: productRows } =
        await loadPurchaseOrderDetailData(id);
  ...
  useEffect(() => {
    if (pageReady) loadDetail();
  }, [pageReady, loadDetail]);

  const handleCancel = async () => {
    if (!order?.id || order.cancelled_at || cancelling || cancellingRef.current)
      return;
    if (
      !window.confirm(
        `Hủy phiếu nhập ${order.code}? Tồn kho sẽ bị trừ lại nếu còn đủ tồn.`,
      )
    )
      return;
    ...
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không hủy được phiếu nhập.",
      );
    } finally {
  ...
  return (
    <>
      <Head>
        <title>Chi tiết phiếu nhập - NPP Hà Công</title>
      </Head>
            ...
            <div>
              <Button asChild variant="outline">
                <Link href="/inventory/purchases">
                  <ArrowLeft className="h-4 w-4" /> Phiếu nhập
                </Link>
              </Button>
              <h1 className="mt-3 text-2xl font-bold">
                {order?.code || "Chi tiết phiếu nhập"}
              </h1>
                ...
                <CardContent className="p-0">
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="grid grid-cols-1 gap-2 border-b border-gray-900 px-4 py-4 last:border-b-0 md:grid-cols-[1.5fr_0.8fr_0.8fr_0.9fr]"
                      >
                        <div>
                        ...
                        <div>
                          {formatInventoryQuantity(item.conversion_to_base_qty)}{" "}
                          gốc
                        </div>
                ...
                <CardContent className="space-y-2 p-4">
                  ...
                  <p>
                    ...
                    <span className="font-semibold">
                      {order.supplier_name || "Chưa có"}
                    </span>
                  </p>
                  <p className="text-xl font-bold">
                    Tổng nhập: {formatMoney(order.total_amount)}
                  </p>
                  <p className="text-gray-400">
                    Trạng thái: {order.cancelled_at ? "Đã hủy" : "Hiệu lực"}
                  </p>
                  {order.note && (
                    <p className="text-gray-300">Ghi chú: {order.note}</p>
                  )}
                </CardContent>
              </Card>
            </div>
