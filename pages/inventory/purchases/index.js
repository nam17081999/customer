    ...
    <>
      ...
      <main className="min-h-screen text-gray-100">
        <div className={`${layoutClasses.shell} space-y-4`}>
          <PageHeader
            title="Phiếu nhập"
            subtitle="Theo dõi nhập kho, tổng nhập và phiếu đã hủy."
            actions={(
          ...
          />
          {error && <div className="rounded-2xl border border-red-900 bg-red-950/25 px-4 py-3 text-red-200">{error}</div>}
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4"><p className="text-sm text-gray-400">Phiếu hiệu lực</p><p className="text-2xl font-bold">{summary.activeOrders}</p></div>
            <div className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4"><p className="text-sm text-gray-400">Tổng nhập</p><p className="text-2xl font-bold">{formatMoney(summary.totalAmount)}</p></div>
            <div className="rounded-2xl border border-red-900 bg-red-950/20 p-4"><p className="text-sm text-red-300">Đã hủy</p><p className="text-2xl font-bold text-red-100">{summary.cancelled}</p></div>
            <div className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4"><p className="text-sm text-gray-400">Tất cả phiếu</p><p className="text-2xl font-bold">{summary.totalOrders}</p></div>
          </div>
          <Card className="rounded-3xl border border-slate-800/80 bg-slate-950/70"><CardContent className="p-0">
            <div className="hidden grid-cols-[0.9fr_1.5fr_1fr_0.9fr_0.8fr] gap-3 border-b border-slate-800 px-4 py-3 text-sm font-semibold text-gray-300 lg:grid">
            ...
            {loading ? <div className="p-4 text-gray-400">Đang tải...</div> : orders.length === 0 ? <div className="p-4 text-gray-400">Chưa có phiếu nhập.</div> : orders.map((order) => (
              <Link key={order.id} href={`/inventory/purchases/${order.id}`} className="grid grid-cols-1 gap-2 border-b border-slate-900/80 px-4 py-4 transition hover:bg-slate-900/50 lg:grid-cols-[0.9fr_1.5fr_1fr_0.9fr_0.8fr] lg:gap-3">
