...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace("/login?from=/inventory/purchases");
      return;
    }
    if (!isAdmin) {
  ...
  const loadOrders = useCallback(async (p = 1) => {
    ...
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không tải được phiếu nhập.",
      );
      setOrders([]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Head>
        <title>Phiếu nhập - NPP Hà Công</title>
      </Head>
      <main className="min-h-full  text-gray-100">
        <div className={`${layoutClasses.shell} space-y-4`}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold">Phiếu nhập</h1>
              <p className="text-base text-gray-400">
                Theo dõi nhập kho, tổng nhập và phiếu đã hủy.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div className="rounded-md border border-gray-800 bg-gray-950 p-4">
              <p className="text-sm text-gray-400">Phiếu hiệu lực</p>
            ...
            <div className="rounded-md border border-red-900 bg-red-950/20 p-4">
              <p className="text-sm text-red-300">Đã hủy</p>
              <p className="text-2xl font-bold text-red-100">
                {summary.cancelled}
              </p>
            </div>
            <div className="rounded-md border border-gray-800 bg-gray-950 p-4">
              <p className="text-sm text-gray-400">Tất cả phiếu</p>
          ...
          <Card>
            <CardContent className="p-0">
              ...
              {loading ? (
                ...
                <div className="p-4 text-gray-400">Chưa có phiếu nhập.</div>
              ) : (
                orders.map((order) => (
                  <Link
                    key={order.id}
                    href={`/inventory/purchases/${order.id}`}
                    className="grid grid-cols-1 gap-2 border-b border-gray-900 px-4 py-4 transition hover:bg-gray-900/60 lg:grid-cols-[0.9fr_1.5fr_1fr_0.9fr_0.8fr] lg:gap-3"
                  >
                    <div>
                      <p className="font-semibold">{order.code}</p>
                      <p className="text-sm text-gray-400">
                        {order.itemCount} dòng hàng
                      </p>
                    </div>
                    ...
                    <div>
                      {order.cancelled_at ? (
                        <span className="rounded-full border border-red-900 bg-red-950/30 px-3 py-1 text-sm text-red-200">
                          Đã hủy
                        </span>
                      ) : (
                        <span className="rounded-full border border-green-900 bg-green-950/30 px-3 py-1 text-sm text-green-200">
                          Hiệu lực
                        </span>
                      )}
                    </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-gray-800 px-4 py-3">
                  <p className="text-sm text-gray-400">
                    Trang {page} / {totalPages} ({totalCount} phiếu)
                  </p>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
