...
export default function StockReportPage() {
  ...
  const lowProducts = useMemo(
    () =>
      products.filter(
        (product) =>
          Number(product.onHandBaseQty || 0) <=
          Number(product.min_stock_base_qty || 0),
      ),
    [products],
  );
  ...
  const filteredMovements = useMemo(
    () => filterStockMovements(enrichedMovements, { query, type }),
    [enrichedMovements, query, type],
  );
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace("/login?from=/inventory/stock");
      return;
    }
    if (!isAdmin) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const {
        ...
        reconciliation,
      } = await loadInventoryStockDashboard();
      ...
      setError(
        err?.operatorMessage ||
          err?.message ||
          "Không tải được báo cáo tồn kho.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleReconciliationCheck = async () => {
    ...
    setError("");
    try {
      const { run, reconciliation } = await runReconciliationCheckAndReload(
        user?.id || null,
      );
      ...
      setError(
        err?.operatorMessage ||
          err?.message ||
          "Không kiểm tra đối soát tồn kho được.",
      );
    } finally {
  ...
  const handleRepair = async () => {
    ...
    if (
      !window.confirm(
        "Sửa tồn hiện tại theo ledger phát sinh? Lợi nhuận lịch sử không bị ghi lại.",
      )
    )
      return;
    setRepairing(true);
    setError("");
    try {
      const { repair, reconciliation } = await repairStockFromLedgerAndReload(
        user?.id || null,
      );
      ...
      setError(
        err?.operatorMessage ||
          err?.message ||
          "Không sửa tồn theo ledger được.",
      );
    } finally {
      setRepairing(false);
    }
  ...
  return (
    <>
      <Head>
        <title>Báo cáo tồn kho - NPP Hà Công</title>
      </Head>
      <main className="min-h-full  text-gray-100 lg:h-screen lg:overflow-hidden">
        <div className="mx-auto flex h-[calc(100dvh-75px)] w-full max-w-[1900px] flex-col px-3 sm:px-4 min-[1900px]:px-8">
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-base text-gray-400">
                Hàng sắp hết và lịch sử phát sinh kho.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {reconciling ? "Đang đối soát..." : "Đối soát"}
              </Button>
              <Button
                type="button"
                variant="outline"
                ...
                disabled={
                  repairing || loading || reconciliationIssues.length === 0
                }
              >
                <Wrench className="h-4 w-4" />{" "}
                {repairing ? "Đang sửa..." : "Sửa theo ledger"}
              </Button>
              <Button
                type="button"
                variant="outline"
          ...
          <div className="shrink-0">
            {error && (
              <div className="my-3 rounded-md border border-red-900 bg-red-950/30 px-4 py-3 text-red-200">
                {error}
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                  ...
                  <p
                    className={
                      reconciliationIssues.length
                        ...
                        : "mt-1 text-2xl font-bold text-green-200"
                    }
                  >
                    {reconciliationIssues.length} lỗi
                  </p>
                </div>
                <div className="rounded-md border border-gray-800 bg-gray-900/50 px-3 py-2.5">
                  <p className="text-xs font-medium uppercase tracking-wider text-gray-500">
                  ...
                  <p className="mt-1 text-2xl font-bold text-red-200">
                    {
                      reconciliationRows.filter(
                        (row) =>
                          row.negative_ledger ||
                          Number(row.replay_on_hand_base_qty || 0) < 0,
                      ).length
                    }
                  </p>
                </div>
                ...
                <div className="rounded-md border border-gray-800 bg-gray-900/50 px-3 py-2.5">
                  ...
                  <p className="mt-1 text-base font-semibold text-gray-100">
                    {reconciliationRun?.status || "Chưa chạy"}
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent className="flex-1 min-h-0 overflow-y-auto space-y-3 p-4">
                ...
                {lowProducts.length === 0 ? (
                  <p className="text-gray-400">
                    Không có hàng dưới tồn tối thiểu.
                  </p>
                ) : (
                  lowProducts.slice(0, 20).map((product) => (
                    <Link
                      ...
                      className="block rounded-md border border-amber-900/50 bg-amber-950/15 px-3 py-2 hover:bg-amber-950/25"
                    >
              ...
              <CardContent className="flex flex-col p-0 min-h-0">
                <div className="shrink-0 border-b border-gray-800 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_220px]">
                    <div className="relative">
                      ...
                      <Input
                        ...
                        placeholder="Tìm hàng hóa hoặc ghi chú"
                      />
                    </div>
                    <select
                  ...
                  <p className="mt-2 text-sm text-gray-400">
                    Hiển thị {filteredMovements.length} / {movements.length}{" "}
                    phát sinh
                  </p>
                  {reconciliationIssues.length > 0 && (
                    <div className="mt-3 rounded-md border border-amber-900/60 bg-amber-950/20 p-3">
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-amber-200">
                        Lỗi đối soát gần đây
                      </p>
                      <ul className="space-y-1.5">
                        {reconciliationIssues.slice(0, 4).map((row) => (
                          ...
                          >
                            ...
                            : {formatIssueCodes(row.issue_codes)}
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto">
                  {loading ? (
                    ...
                    <div className="p-4 text-gray-400">
                      Chưa có phát sinh kho.
                    </div>
                  ) : (
                    filteredMovements.map((row) => (
                      <div
                        key={row.id}
                        className="grid grid-cols-1 gap-2 border-b border-gray-900 px-4 py-4 last:border-b-0 lg:grid-cols-[1.4fr_0.8fr_0.8fr_1fr]"
                      >
                        <div>
                        ...
                        <div
                          className={
                            Number(row.quantity_base || 0) >= 0
                              ? "text-base font-semibold text-green-200"
                              : "text-base font-semibold text-red-200"
                          }
                        >
                          {formatInventoryQuantity(row.quantity_base)}
                        </div>
