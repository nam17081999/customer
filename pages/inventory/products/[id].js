...
export default function ProductEditPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace(`/login?from=/inventory/products/${id || ""}`);
      return;
    }
    if (!isAdmin) {
  ...
  const loadProduct = useCallback(async () => {
    ...
    try {
      ...
      setUnitForms(
        Object.fromEntries(
          (row?.units || []).map((unit) => [unit.id, buildUnitForm(unit)]),
        ),
      );
  ...
  useEffect(() => {
    if (pageReady) loadProduct();
  }, [pageReady, loadProduct]);
  ...
  const updateNewUnitForm = (field, value) =>
    setNewUnitForm((prev) => ({ ...prev, [field]: value }));
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không lưu được hàng hóa.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleUnitSubmit = async (unit) => {
    ...
    setMessage("");
    try {
      await saveProductUnitFromForm(
        unit.id,
        unitForms[unit.id] || buildUnitForm(unit),
      );
      setMessage("Đã lưu đơn vị.");
      await loadProduct();
    } catch (err) {
      setError(
        err?.operatorMessage || err?.message || "Không lưu được đơn vị.",
      );
    } finally {
      setUnitSubmittingId("");
    }
  ...
  const handleAddUnit = async () => {
    ...
    try {
      ...
      setError(
        err?.operatorMessage || err?.message || "Không thêm được đơn vị.",
      );
    } finally {
      setUnitSubmittingId("");
    }
  ...
  return (
    <>
      <Head>
        <title>Sửa hàng hóa - NPP Hà Công</title>
      </Head>
      <main className="min-h-full  text-gray-100">
        <form
          className={`${layoutClasses.formShell} space-y-4`}
        ...
        >
            ...
            <div>
              <Button asChild variant="outline">
                <Link href="/inventory/products">
                  <ArrowLeft className="h-4 w-4" /> Hàng hóa
                </Link>
              </Button>
              <h1 className="mt-3 text-2xl font-bold">Sửa hàng hóa</h1>
              <p className="text-base text-gray-400">
                {product?.name || "Thông tin mặt hàng"}
              </p>
            </div>
          ...
          {(error || message) && (
            <div
              className={`rounded-md border px-4 py-3 ${error ? "border-red-900 bg-red-950/30 text-red-200" : "border-green-900 bg-green-950/30 text-green-200"}`}
            >
              {error || message}
            </div>
          )}
                ...
                <CardContent className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2">
                  ...
                  <label className="block space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm("defaultSalePrice", e.target.value)
                      }
                    />
                  </label>
                  <label className="block space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm("defaultPurchasePrice", e.target.value)
                      }
                    />
                  </label>
                  <label className="block space-y-1">
                    <span className="text-sm text-gray-300">Giá bán lẻ</span>
                    <Input
                      type="number"
                      min="0"
                      value={form.retailPrice ?? product?.retail_price ?? ""}
                      onChange={(e) =>
                        updateForm("retailPrice", e.target.value)
                      }
                    />
                  </label>
                  <label className="block space-y-1">
                    <span className="text-sm text-gray-300">Giá bán xỉ</span>
                    <Input
                      type="number"
                      min="0"
                      value={
                        form.wholesalePrice ?? product?.wholesale_price ?? ""
                      }
                      onChange={(e) =>
                        updateForm("wholesalePrice", e.target.value)
                      }
                    />
                  </label>
                  <label className="block space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm("minStockBaseQty", e.target.value)
                      }
                    />
                  </label>
                  <label className="flex items-center gap-3 rounded-md border border-gray-800 bg-gray-900/70 px-3 py-2">
                    <input
                      type="checkbox"
                      checked={form.active}
                ...
                <Card>
                  <CardContent className="space-y-2 p-4">
                    ...
                    <p className="text-gray-400">
                      Giá vốn: {formatMoney(product.avgCostPerBaseUnit)} /{" "}
                  ...
                  <CardContent className="space-y-3 p-4">
                    ...
                    {(product.units || []).map((unit) => {
                      ...
                      return (
                        <div
                          key={unit.id}
                          className="space-y-3 rounded-md border border-gray-800 bg-gray-900/50 p-3"
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span
                              className={`rounded-full border px-2 py-0.5 text-sm ${unitForm.active ? "border-green-900 bg-green-950/30 text-green-200" : "border-red-900 bg-red-950/30 text-red-200"}`}
                            >
                          ...
                          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            ...
                            <label className="block space-y-1">
                              <span className="text-sm text-gray-300">
                                Quy đổi về {product.base_unit_name}
                              </span>
                              <Input
                                type="number"
                                min="0.001"
                                value={unitForm.conversionToBaseQty}
                            ...
                            <label className="block space-y-1">
                              ...
                              <Input
                                type="number"
                                min="0"
                                value={unitForm.defaultSalePrice}
                                onChange={(e) =>
                                  updateUnitForm(
                                    unit.id,
                                    "defaultSalePrice",
                            ...
                            <label className="block space-y-1">
                              ...
                              <Input
                                type="number"
                                min="0"
                                value={unitForm.defaultPurchasePrice}
                                onChange={(e) =>
                                  updateUnitForm(
                                    unit.id,
                                    "defaultPurchasePrice",
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <Button
                              type="button"
                              variant="outline"
                              onClick={() => handleUnitSubmit(unit)}
                              disabled={unitSubmittingId === unit.id}
                            >
                              {unitSubmittingId === unit.id
                                ? "Đang lưu..."
                                : "Lưu đơn vị"}
                            </Button>
                          </div>
                    ...
                    <div className="space-y-3 rounded-md border border-sky-900/50 bg-sky-950/10 p-3">
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <label className="block space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateNewUnitForm("unitName", e.target.value)
                            }
                        ...
                        <label className="block space-y-1">
                          ...
                          <Input
                            type="number"
                            min="0.001"
                            value={newUnitForm.conversionToBaseQty}
                            onChange={(e) =>
                              updateNewUnitForm(
                                "conversionToBaseQty",
                                e.target.value,
                        ...
                        <label className="block space-y-1">
                          <span className="text-sm text-gray-300">Giá bán</span>
                          <Input
                            type="number"
                            min="0"
                            value={newUnitForm.defaultSalePrice}
                            onChange={(e) =>
                              updateNewUnitForm(
                                "defaultSalePrice",
                                e.target.value,
                        ...
                        <label className="block space-y-1">
                          ...
                          <Input
                            type="number"
                            min="0"
                            value={newUnitForm.defaultPurchasePrice}
                            onChange={(e) =>
                              updateNewUnitForm(
                                "defaultPurchasePrice",
                                e.target.value,
                      ...
                      <Button
                        ...
                        disabled={unitSubmittingId === "new"}
                      >
                        {unitSubmittingId === "new"
                          ? "Đang thêm..."
                          : "Thêm đơn vị"}
                      </Button>
                    </div>
