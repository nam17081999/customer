...
test('user gửi edit report với payload đã chuẩn hóa và tọa độ mới', async ({ page }) => {
  ...
  await page.waitForURL('**/store/report/report-edit-1/edit?*')
  await expect(page.getByRole('heading', { name: 'Sửa cửa hàng' })).toBeVisible()

  await page.getByLabel('Tên cửa hàng').fill('tạp hóa minh anh')
  await page.getByRole('button', { name: 'Tiếp theo →' }).click()
  await page.getByLabel('Địa chỉ cụ thể').fill('xóm chợ mới')
  await page.getByLabel('Số điện thoại').fill('+84901234568')
  await page.getByLabel('Ghi chú').fill('mở từ 5 giờ sáng')
  await page.getByRole('button', { name: 'Tiếp theo →' }).click()
  await page.getByRole('button', { name: 'Lấy lại vị trí' }).click()
  await page.getByRole('button', { name: 'Gửi báo cáo' }).click()

  await page.waitForURL('**/')
  await expect(page.getByText('Đã gửi báo cáo. Admin sẽ xem xét và cập nhật.')).toBeVisible()
...
test('user gửi reason-only report không kèm proposed_changes', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Sai tên Chọn' }).click()
  ...
  expect(reportPayload.reason_codes).toEqual(['wrong_name', 'wrong_phone'])
})

test('user thấy lý do báo cáo đúng thứ tự mới', async ({ page }) => {
  await setupReportFlow(page, {
    stores: [buildStore({ id: 'report-reason-order-1' })],
  })

  await page.goto('/store/report/report-reason-order-1?from=%2F')

  const reasonButtons = page.locator('button').filter({ hasText: /Sai (tên|địa chỉ|số điện thoại|vị trí)/ })
  await expect(reasonButtons).toHaveCount(4)
  await expect(reasonButtons.nth(0)).toContainText('Sai tên')
  await expect(reasonButtons.nth(1)).toContainText('Sai địa chỉ')
  await expect(reasonButtons.nth(2)).toContainText('Sai số điện thoại')
  await expect(reasonButtons.nth(3)).toContainText('Sai vị trí')
})
...
test('user không thể gửi edit report với số điện thoại sai hoặc không có thay đổi', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Sửa thông tin' }).click()
  await page.waitForURL('**/store/report/report-guard-1/edit?*')
  await page.getByRole('button', { name: 'Tiếp theo →' }).click()
...
test('user bấm quay lại ở màn sửa report sẽ về màn report chi tiết', async ({ page }) => {
  await setupReportFlow(page, {
    stores: [buildStore({ id: 'report-back-1', name: 'Điểm Bán Quay Lại Report' })],
  })

  await page.goto('/store/report/report-back-1?from=%2F')
  await page.getByRole('link', { name: 'Sửa thông tin' }).click()
  await page.waitForURL('**/store/report/report-back-1/edit?*')
  await expect(page.getByRole('heading', { name: 'Sửa cửa hàng' })).toBeVisible()

  await page.getByRole('button', { name: 'Quay lại' }).click()

  await page.waitForURL('**/store/report/report-back-1')
  await expect(page.getByRole('heading', { name: 'Báo cáo cửa hàng' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Điểm Bán Quay Lại Report' })).toBeVisible()
})
...
test('admin approve edit report cập nhật store rồi mới duyệt report', async ({ page }) => {
  ...
  await page.waitForURL('**/store/reports/pending-edit-1')
...
test('admin đánh dấu reason-only report không patch stores', async ({ page }) => {
  ...
  await page.goto('/store/reports')
  await page.getByRole('link', { name: 'Xem chi tiết' }).click()
  await page.waitForURL('**/store/reports/pending-reason-1')
