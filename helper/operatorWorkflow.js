...
export function buildCommandSearchResults({ query = '', actions = OPERATOR_QUICK_ACTIONS, products = [], customers = [], orders = [] } = {}) {
  const actionResults = (Array.isArray(actions) ? actions : [])
    .filter((action) => includesQuery([action.label, action.href, action.shortcut], query))
    .map((action) => ({ type: 'action', id: action.key, label: action.label, subtitle: action.shortcut, href: action.href }))

  const productResults = (Array.isArray(products) ? products : [])
    .filter((product) => includesQuery([product.name, product.sku, product.category], query))
    .slice(0, 8)
    .map((product) => ({ type: 'product', id: product.id, label: product.name, subtitle: product.sku || 'Hàng hóa', href: `/inventory/products/${product.id}` }))

  const customerResults = (Array.isArray(customers) ? customers : [])
    .filter((store) => includesQuery([store.name, store.phone, store.address, store.ward, store.district], query))
    .slice(0, 8)
    .map((store) => ({ type: 'customer', id: store.id, label: store.name, subtitle: [store.phone, store.ward, store.district].filter(Boolean).join(' · '), href: `/store/${store.id}` }))

  const orderResults = (Array.isArray(orders) ? orders : [])
    .filter((order) => includesQuery([order.code, order.customer_name], query))
    .slice(0, 8)
    .map((order) => ({ type: 'order', id: order.id, label: order.code || 'Đơn hàng', subtitle: order.customer_name || 'Đơn bán', href: `/orders/${order.id}` }))

  return [...actionResults, ...productResults, ...customerResults, ...orderResults].slice(0, 20)
}
